...
# Constant paths to temporary folders will be created
# EXE - all folders temporary/SOURCE - only ROOT amd TEMP temporary
ROOT_FOLDER = None
WEB_FOLDER = None
TEMP_FOLDER = None
...
def init_app():
    """
    Create temporary folders at the application start-up.
    """
    global ROOT_FOLDER, WEB_FOLDER, TEMP_FOLDER
    # Running EXE file
    # Check for PyInstaller folder
    if hasattr(sys, "_MEIPASS"):
        # Ask for temp folder
        ROOT_FOLDER = tempfile.mkdtemp()
        # Path to read only web folder from exe
        web_src = os.path.join(sys._MEIPASS, "web")
        # Path to where copied temporary web folder will be placed
        web_dst = os.path.join(ROOT_FOLDER, "web")
        WEB_FOLDER = web_dst
        # Copy the web folder - available write
        shutil.copytree(web_src, web_dst)
        # Create temp folder inside for output
        TEMP_FOLDER = os.path.join(WEB_FOLDER, "temp")
    ...
    # Running from source
        ...
        WEB_FOLDER = "web"
        TEMP_FOLDER = os.path.join(WEB_FOLDER, "temp")
        ROOT_FOLDER = os.path.join(WEB_FOLDER, "temp")
        # Create temp folder inside for output
        os.makedirs(TEMP_FOLDER, exist_ok=True)
    print(f"Temporary folder '{ROOT_FOLDER}' created.\n")
    return WEB_FOLDER
...
def quit_app():
    ...
    try:
        if os.path.exists(ROOT_FOLDER):
            ...
            print(f"Temporary folder '{ROOT_FOLDER}' deleted.")

    except Exception as e:
        print(f"Error while deleting temporary folder: {e}")

    print("Application closed.")
    os._exit(0) # Exit the app
