...
def process_fasta(file, midi_path, wav_path, mapping_algorithm, midi_algorithm, synthesize_algorithm, lines_count, bpm):
    """
    Read the FASTA file and use selected algorithms and synthesize method on the nucleotide sequence. The FASTA processor servers as adapter
    ...
    Arguments:
        ...
        lines_count (int): Number of lines that would be converted to notes from original file with nucleotide sequence.
    ...
    for line in file.splitlines():
        ...
        if lines_count is not None and line_count > lines_count:
            break
        # SKip header line
        if line.startswith(">"):
            continue
    ...
    midi_algorithm(full_sequence, midi_path, bpm)
